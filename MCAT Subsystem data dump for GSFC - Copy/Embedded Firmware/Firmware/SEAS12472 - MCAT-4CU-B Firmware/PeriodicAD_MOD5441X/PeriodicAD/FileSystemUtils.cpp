/* Rev:$Revision: 1.2 $ */
/* Copyright $Date: 2012/03/26 19:52:42 $ */ 
/*******************************************************************************
 *
 *   Copyright 2007-2012 NetBurner, Inc.  ALL RIGHTS RESERVED
 *
...
int OpenMMCFlash(int drv)
{
	...
	if ( get_cd(drv) == 0 )
	{
	   while ( get_cd(drv) == 0 )
	   {
		if(drv==MMC_OFF_BOARD) 
		  iprintf( "No OffBoard MMC/SD card detected. Insert a card and then press " );
		else
		  iprintf( "No OnBoard MMC/SD card detected. Insert a card and then press " );
	...
	if(drv==MMC_OFF_BOARD) rv= f_mountfat(drv, mmc_initfunc, F_MMC_DRIVE0 );
   	...
   	if(drv==MMC_ON_BOARD) rv= f_mountfat(drv, mmc_initfunc, F_MMC_DRIVE1 );
